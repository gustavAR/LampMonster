...
public:
	NaiveBayes(CategoryPaths paths)
	{
		...
		for(auto path = paths.cbegin(); path != paths.cend(); path++) //for each training path in paths
			categories[i++] = *(new Category(path->first,BuildBagOfWords(path->second)));	//Build and fill a category with a bag of words
	}

	virtual string Classify(vector<string> words)
	{
		Category fittest = categories.begin;	//Set first category as default
		int maxProb = 0;						//Set current maximum category probability to 0
		for(Category c : categories) {			//For each category
			int curProb = (NBC(1/categories.size, c.second, words, prior)); //Calculate probability for the words belonging to that category
		...
		return fittest.first;					//Returns the name of the most probable category
	}
	/*
	double NBC(double probOfClass, const map<string,int>& vocabulary, const vector<string>& words, double prior)
	{
